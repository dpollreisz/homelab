resource "authentik_flow" "pollreisz-authentication" {
  name               = "Pollreisz Authentication Flow"
  slug               = "pollreisz-authentication-flow"
  title              = "Welcome to Pollreisz!"
...
locals {
  stage_bindings = {
    0 = authentik_stage_identification.pollreisz-identity-stage.id
...
resource "authentik_flow_stage_binding" "dh-sb-identity" {
  ...
  target   = authentik_flow.pollreisz-authentication.uuid
  stage    = each.value
  order    = each.key
}
